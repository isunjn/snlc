...
function S_START(ch: Char): StateFnResult {
  ...
  if (isDelimiter.test(ch)) {
    switch (ch) {
      case "+": return new Token(LINE, "PLUS");
      case "-": return new Token(LINE, "MINUS");
      case "*": return new Token(LINE, "MULTI");
      case "/": return new Token(LINE, "DEVIDE");
      case "(": return new Token(LINE, "L_PAREN");
      case ")": return new Token(LINE, "R_PAREN");
      case "[": return new Token(LINE, "L_SQUARE");
      case "]": return new Token(LINE, "R_SQUARE");
      case ";": return new Token(LINE, "SEMI");
      case "<": return new Token(LINE, "LT");
      case "=": return new Token(LINE, "EQ");
      case ",": return new Token(LINE, "COMMA")
      case "EOI": return new Token(LINE, "EOI");
    }
  }
...
function S_INDOT(ch: Char): StateFnResult {
  if ("." === ch) return new Token(LINE, "RANGE");
  goBack(ch === "\n", ch === "EOI");
...
function S_INENDCHAR(ch: Char, str: Char[]): StateFnResult {
  if ("'" === ch) return new Token(LINE, "CHARC", str.pop()!);
